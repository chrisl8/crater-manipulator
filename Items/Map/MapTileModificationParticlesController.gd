...
func DestroyCellLocal(Position: Vector2i, ID: Vector2i) -> void:
	if ID != Vector2i(-1, -1):
		...
		if TileImage.get_width() > 0 and TileImage.get_height() > 0:
			var PointsToSample: int = ParticlesPerTile
			var PointsSampled: Array = []
			while PointsToSample > 0:
				PointsToSample -= 1
				#Should store all possible points in variable on startup and pick randomly later, duplicates can be addressed or ignored
